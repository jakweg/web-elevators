...
const waitingPassengersListDiv = document.getElementById(
...
system.addEventListener("elevator-updated", (elevator: Elevator) => {
  ...
  (elevatorDiv.querySelector(
    ".current-floor-value"
  ) as HTMLDivElement).innerText = `${elevator.currentFloor}`;
  (elevatorDiv.querySelector(
    ".direction-value"
  ) as HTMLDivElement).innerText = `${formatDirection(elevator.direction)}`;
...
system.addEventListener(
  "passenger-taken",
  ({ passenger, elevator }: ElevatorAndPassenger) => {
    document.getElementById(`waiting-passenger-id-${passenger.id}`).remove();
    const listOfInsiders = document.querySelector(
      `#elevator-id-${elevator.id} .passengers-inside-list`
    );
